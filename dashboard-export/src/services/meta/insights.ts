...
export async function getInsights(params: InsightsParams): Promise<CampaignInsight[]> {
  ...
  if (params.daily) queryParams.time_increment = '1'
  ...
  return (resp.data ?? []).map(row => {
    ...
    return {
      ...
      date:            row.date_start,
      spend, impressions, reach, clicks, linkClicks, ctr, cpm, frequency,
      purchases, leads, initiateCheckout, revenue,
...
export function aggregateInsights(rows: CampaignInsight[]) {
  ...
  return {
    spend, impressions, reach, clicks, linkClicks, purchases, leads,
    initiateCheckout, revenue, landingPageViews, frequency,
    roas:                    spend > 0            ? revenue / spend               : 0,
    connectRate:             clicks > 0           ? (landingPageViews / clicks) * 100 : 0,
