...
const fmtDecimal  = (v: number) => v.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
...
function scoreCampaigns(rows: CampaignRow[]) {
  ...
  const cpxVals = rows.map(r => r.purchases > 0 ? r.costPerPurchase : r.leads > 0 ? r.costPerLead : r.spend)
  const cpxN    = norm(cpxVals).map(v => 1 - v)
...
function exportCampaignsXLSX(rows: CampaignRow[], startDate: string, endDate: string) {
  ...
  const headers = ['#', 'Campanha', 'Score (0-100)', 'ROAS', 'Tx Conv. Compra (%)', 'Tx Conv. Lead (%)', 'Custo/Compra (R$)', 'Custo/Lead (R$)', 'Compras', 'Leads', 'Investido (R$)', 'Receita (R$)', 'Alcance', 'Impressões', 'CTR (%)', 'CPM (R$)', 'CPC (R$)', 'Frequência', 'Connect Rate (%)', 'Init. Checkout']
  const dataRows = ranked.map((r, i) => [i+1, r.campaignName, r.score, +r.roas.toFixed(2), +r.purchaseRate.toFixed(2), +r.leadRate.toFixed(2), +r.costPerPurchase.toFixed(2), +r.costPerLead.toFixed(2), r.purchases, r.leads, +r.spend.toFixed(2), +r.revenue.toFixed(2), r.reach, r.impressions, +r.ctr.toFixed(2), +r.cpm.toFixed(2), +r.cpc.toFixed(2), +r.frequency.toFixed(2), +r.connectRate.toFixed(2), r.initiateCheckout])
  const ws = XLSX.utils.aoa_to_sheet([[`Campanhas no Período — ${startDate} a ${endDate}`], [], headers, ...dataRows])
  ws['!cols'] = [{wch:4},{wch:40},{wch:13},{wch:8},{wch:18},{wch:16},{wch:16},{wch:14},{wch:9},{wch:8},{wch:14},{wch:13},{wch:11},{wch:11},{wch:8},{wch:10},{wch:10},{wch:11},{wch:16},{wch:14}]
...
function aggregate(rows: CampaignInsight[]): AggregatedData | null {
  ...
  const frequency        = impressions > 0 ? rows.reduce((s, r) => s + r.frequency * r.impressions, 0) / impressions : 0
  return {
    spend, impressions, reach, clicks, linkClicks, purchases, leads, initiateCheckout, revenue,
...
function CampaignFilter({ campaigns, selected, onChange }: { campaigns: MetaCampaign[]; selected: string[]; onChange: (ids: string[]) => void }) {
  ...
  const filtered = campaigns.filter(c => {
    ...
    const matchStatus = statusFilter === 'all' ? true : statusFilter === 'active' ? effStatus === 'ACTIVE' : effStatus !== 'ACTIVE'
    return matchSearch && matchStatus
  })

  const label = selected.length === 0 ? 'Todas as campanhas' : selected.length === 1 ? campaigns.find(c => c.id === selected[0])?.name ?? '1 selecionada' : `${selected.length} campanhas`
  const STATUS_TABS: { id: StatusFilter; label: string }[] = [{ id: 'all', label: 'Todas' }, { id: 'active', label: 'Ativas' }, { id: 'inactive', label: 'Pausadas' }]

  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(v => !v)} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-surface-800 border border-surface-700 text-sm text-gray-300 hover:border-surface-600 transition-colors min-w-[200px]">
        <span className="flex-1 text-left truncate">{label}</span>
        <ChevronDown className={`w-4 h-4 flex-shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="absolute top-full mt-2 left-0 z-50 w-80 bg-surface-800 border border-surface-700 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden">
          <div className="p-2.5 border-b border-surface-700">
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-surface-700 border border-surface-600 focus-within:border-indigo-500/60 transition-colors">
              ...
              <input ref={inputRef} type="text" placeholder="Buscar campanha..." value={search} onChange={e => setSearch(e.target.value)} className="flex-1 bg-transparent text-sm text-gray-200 placeholder-gray-500 outline-none min-w-0" />
              {search && <button onClick={() => setSearch('')} className="text-gray-500 hover:text-gray-300 transition-colors"><X className="w-3.5 h-3.5" /></button>}
            </div>
          </div>
          <div className="flex gap-1 p-2 border-b border-surface-700">
            {STATUS_TABS.map(tab => (
              <button key={tab.id} onClick={() => setStatus(tab.id)} className={`flex-1 text-xs py-1.5 rounded-lg font-medium transition-colors ${statusFilter === tab.id ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/40' : 'bg-surface-700 text-gray-400 hover:text-gray-200 hover:bg-surface-600 border border-transparent'}`}>{tab.label}</button>
            ))}
          </div>
          <div className="flex gap-1 p-2 border-b border-surface-700">
            <button onClick={() => onChange(filtered.map(c => c.id))} className="flex-1 text-xs py-1.5 rounded-lg bg-surface-700 text-gray-300 hover:bg-surface-600 transition-colors">Selecionar visíveis</button>
            <button onClick={() => onChange([])} className="flex-1 text-xs py-1.5 rounded-lg bg-surface-700 text-gray-300 hover:bg-surface-600 transition-colors">Limpar</button>
          </div>
          <div className="max-h-56 overflow-y-auto py-1">
            {filtered.length === 0 && <p className="text-xs text-gray-500 px-4 py-3 text-center">Nenhuma campanha encontrada</p>}
            {filtered.map(c => (
              <button key={c.id} onClick={() => toggle(c.id)} className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-surface-700 transition-colors text-left">
                <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 border transition-colors ${selected.includes(c.id) ? 'bg-indigo-500 border-indigo-500' : 'border-surface-600'}`}>
...
function MetricCard({ metric, value, isDragging, isOver, onDragStart, onDragOver, onDrop, onDragEnd, onRemove, customLabel, onLabelChange, metricId, prevValue }: {
  metric: MetricDef; value: number; isDragging: boolean; isOver: boolean
  onDragStart: () => void; onDragOver: (e: React.DragEvent) => void; onDrop: () => void; onDragEnd: () => void; onRemove: () => void
  customLabel?: string; onLabelChange: (id: string, label: string) => void; metricId: string; prevValue?: number
}) {
  const [editing,   setEditing]   = useState(false)
  const [editValue, setEditValue] = useState(customLabel ?? metric.label)
  const displayLabel = customLabel || metric.label
  const handleLabelSubmit = () => { setEditing(false); if (editValue.trim()) onLabelChange(metricId, editValue.trim()) }
  ...
  const freqColor   = isFrequency ? value >= 4 ? 'text-red-500' : value >= 3 ? 'text-orange-500' : '' : ''
  ...
  return (
    <div draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop} onDragEnd={onDragEnd}
      ...
      style={{ background: 'var(--s-850)', border: isDragging ? '1px solid var(--t-border)' : isOver ? '1px solid rgba(249,115,22,0.5)' : '1px solid var(--t-border)', opacity: isDragging ? 0.4 : 1, transform: isDragging ? 'scale(0.95)' : isOver ? 'scale(1.02)' : 'scale(1)' }}>
      <div className="absolute top-3 left-3 cursor-grab active:cursor-grabbing" style={{ color: 'var(--t-3)' }}><GripVertical className="w-4 h-4" /></div>
      <button onClick={onRemove} className="absolute top-3 right-3 p-1 rounded-lg transition-colors" style={{ color: 'var(--t-3)' }} onMouseEnter={e => (e.currentTarget.style.color = 'var(--t-1)')} onMouseLeave={e => (e.currentTarget.style.color = 'var(--t-3)')}><X className="w-3.5 h-3.5" /></button>
      <div className="mt-1 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${metric.color}`} style={{ background: 'var(--s-800)' }}><Icon className="w-4.5 h-4.5" /></div>
          {freqHigh && <span title="Frequência alta — considere pausar ou renovar criativos"><AlertTriangle className="w-4 h-4 text-orange-400" /></span>}
        </div>
        <div>
          {editing ? (
            <input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)}
              onBlur={handleLabelSubmit} onKeyDown={e => { if (e.key === 'Enter') handleLabelSubmit(); if (e.key === 'Escape') setEditing(false) }}
              className="text-xs font-medium bg-transparent outline-none w-full mb-1" style={{ color: '#f97316', borderBottom: '1px solid #f97316' }} />
          ) : (
            <p className="text-xs font-medium mb-1 cursor-pointer transition-colors" style={{ color: 'var(--t-3)' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#f97316')} onMouseLeave={e => (e.currentTarget.style.color = 'var(--t-3)')}
              onDoubleClick={() => { setEditValue(displayLabel); setEditing(true) }} title="Clique duplo para editar o rótulo">{displayLabel}</p>
          )}
          <p className={`text-2xl font-bold leading-none ${freqColor}`} style={!freqColor ? { color: 'var(--t-1)' } : {}}>{formatValue(value, metric.format)}</p>
          {showDelta && prevValue! !== 0 && (
            <div className={`flex items-center gap-1 mt-1.5 text-xs font-medium ${delta > 0 ? 'text-emerald-500' : delta < 0 ? 'text-red-500' : ''}`} style={delta === 0 ? { color: 'var(--t-3)' } : {}}>
...
function AddMetricModal({ visibleIds, onAdd, onClose }: { visibleIds: string[]; onAdd: (id: string) => void; onClose: () => void }) {
    ...
    <>
      ...
      <div className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[420px] rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--t-border)' }}>
          ...
          <button onClick={onClose} className="p-1 rounded transition-colors" style={{ color: 'var(--t-3)' }} onMouseEnter={e => (e.currentTarget.style.color = 'var(--t-1)')} onMouseLeave={e => (e.currentTarget.style.color = 'var(--t-3)')}><X className="w-4 h-4" /></button>
        </div>
        <div className="p-3 max-h-80 overflow-y-auto">
          {hidden.length === 0 ? (
            ...
            <div className="space-y-1">
              {hidden.map(m => { const Icon = m.icon; return (
                <button key={m.id} onClick={() => { onAdd(m.id); onClose() }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors text-left" onMouseEnter={e => (e.currentTarget.style.background = 'var(--s-800)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${m.color}`} style={{ background: 'var(--s-800)' }}><Icon className="w-4 h-4" /></div>
                  <div><p className="text-sm" style={{ color: 'var(--t-1)' }}>{m.label}</p><p className="text-xs capitalize" style={{ color: 'var(--t-3)' }}>{m.format}</p></div>
...
function Th({ label, sortKey, currentKey, dir, onSort }: { label: string; sortKey: string; currentKey: string; dir: SortDir; onSort: (key: string) => void }) {
  const active = sortKey === currentKey
  return (
    <th onClick={() => onSort(sortKey)} className="px-4 py-3 text-left text-xs font-semibold cursor-pointer transition-colors whitespace-nowrap select-none" style={{ color: 'var(--t-3)' }} onMouseEnter={e => (e.currentTarget.style.color = 'var(--t-1)')} onMouseLeave={e => (e.currentTarget.style.color = 'var(--t-3)')}>
      <div className="flex items-center gap-1">{label}{active ? dir === 'asc' ? <ArrowUp className="w-3 h-3" style={{ color: '#f97316' }} /> : <ArrowDown className="w-3 h-3" style={{ color: '#f97316' }} /> : <ArrowUpDown className="w-3 h-3 opacity-40" />}</div>
    </th>
  )
}
...
function FunnelSection({ data }: { data: AggregatedData }) {
  ...
  return (
    <div className="rounded-2xl p-5" style={{ background: 'var(--s-850)', border: '1px solid var(--t-border)' }}>
      ...
      <div>
        {steps.map((step, i) => {
          const rate = i > 0 && steps[i - 1].value > 0 ? (step.value / steps[i - 1].value) * 100 : null
          return (
            <div key={step.label} className="flex items-center justify-between py-2.5 last:border-0" style={{ borderBottom: i < steps.length - 1 ? '1px solid var(--t-border)' : 'none' }}>
              ...
              <div className="flex items-center gap-3">
                {rate !== null && <span className="text-xs" style={{ color: 'var(--t-3)' }}>↓ {rate.toFixed(1)}%</span>}
                <span className="text-sm font-semibold tabular-nums w-24 text-right" style={{ color: 'var(--t-1)' }}>{fmtNumber(step.value)}</span>
              </div>
            </div>
          )
        })}
...
function PaceSection({ daily, endDate }: { daily: CampaignInsight[]; endDate: string }) {
  ...
  const items = [
    { label: 'Investimento', proj: fmtCurrency(avgSpend * diasRestantes),              avg: `${fmtCurrency(avgSpend)}/dia`       },
    { label: 'Compras',      proj: fmtNumber(Math.round(avgPurchases * diasRestantes)), avg: `${fmtDecimal(avgPurchases)}/dia`   },
    { label: 'Leads',        proj: fmtNumber(Math.round(avgLeads * diasRestantes)),     avg: `${fmtDecimal(avgLeads)}/dia`       },
    { label: 'Receita',      proj: fmtCurrency(avgRevenue * diasRestantes),             avg: `${fmtCurrency(avgRevenue)}/dia`    },
  ]
  return (
    <div className="rounded-2xl px-5 py-4" style={{ background: 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.2)' }}>
      <div className="flex items-center gap-2 mb-3.5">
        ...
        <span className="text-xs" style={{ color: 'var(--t-3)' }}>— {diasRestantes} {diasRestantes === 1 ? 'dia restante' : 'dias restantes'} no período, com base em {diasPassados} dias de dados</span>
      </div>
        ...
        {top.map((r, i) => (
          <div key={r.campaignId} className="px-5 py-3" style={{ borderBottom: i < top.length - 1 ? '1px solid var(--t-border)' : 'none' }}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-medium truncate flex-1" style={{ color: 'var(--t-1)' }} title={r.campaignName}>{r.campaignName}</span>
            ...
            <p className="text-[11px] ml-5 leading-relaxed" style={{ color: 'var(--t-3)' }}>
              {fmtNumber(r.purchases)} compras{r.costPerPurchase > 0 && <> · {fmtCurrency(r.costPerPurchase)}/compra</>}{' '}· {fmtPercent(r.purchaseRate)} conv.
              <span className="mx-1.5" style={{ color: 'var(--t-border)' }}>|</span>
              {fmtNumber(r.leads)} leads{r.costPerLead > 0 && <> · {fmtCurrency(r.costPerLead)}/lead</>}{' '}· {fmtPercent(r.leadRate)} conv.
            </p>
          </div>
        ))}
      </div>
...
type ChartMetricDef = { key: keyof CampaignInsight; label: string; color: string; format: 'currency' | 'number' | 'percent' | 'decimal' }
...
function EvolutionChart({ daily, workspaceId }: { daily: CampaignInsight[]; workspaceId: string }) {
  ...
  useEffect(() => {
    ...
    if (saved) { try { const p = JSON.parse(saved) as string[]; if (p.length >= 1 && p.length <= 4) { setSelectedKeys(p); setActiveKey(p[0]) } } catch {} }
  }, [storageKey])
  ...
  const handlePickerClick = (key: string) => {
    if (selectedKeys.includes(key)) { setActiveKey(key) } else {
      const next = selectedKeys.map(k => k === activeKey ? key : k)
      setSelectedKeys(next); setActiveKey(key)
      localStorage.setItem(storageKey, JSON.stringify(next))
    }
  }

  const selectedDefs = selectedKeys.map(k => ALL_CHART_METRICS.find(m => (m.key as string) === k)).filter((m): m is ChartMetricDef => !!m)
  const data = [...daily].filter(r => r.date).sort((a, b) => (a.date ?? '').localeCompare(b.date ?? '')).map(row => ({
    label: new Date(row.date! + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }),
    ...(Object.fromEntries(selectedDefs.map(m => [m.key as string, row[m.key] as number ?? 0]))),
  }))
    ...
    <div className="rounded-2xl p-6" style={{ background: 'var(--s-850)', border: '1px solid var(--t-border)' }}>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="relative" ref={pickerRef}>
          <button onClick={() => setShowPicker(v => !v)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs transition-colors" style={{ background: 'var(--s-800)', borderColor: 'var(--t-border)', color: 'var(--t-2)' }}>
            <Settings2 className="w-3.5 h-3.5" />Métricas
          </button>
          {showPicker && (
            <div className="absolute right-0 top-full mt-2 z-50 w-72 rounded-2xl overflow-hidden" style={{ background: 'var(--s-900)', border: '1px solid var(--t-border)', boxShadow: '0 16px 48px rgba(0,0,0,0.15)' }}>
              <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--t-border)' }}>
                ...
                <p className="text-[11px] mt-0.5" style={{ color: 'var(--t-3)' }}>Clique em uma não selecionada para substituir <span style={{ color: activeDef?.color }}>{activeDef?.label}</span></p>
              </div>
              <div className="p-2 grid grid-cols-2 gap-1 max-h-72 overflow-y-auto">
                {ALL_CHART_METRICS.map(m => {
                  ...
                  return (
                    <button key={m.key as string} onClick={() => handlePickerClick(m.key as string)} className="flex items-center gap-2 px-2.5 py-2 rounded-xl text-left text-xs transition-colors"
                      style={{ background: sel ? (active ? 'var(--s-800)' : 'var(--s-850)') : 'transparent', color: sel ? 'var(--t-1)' : 'var(--t-3)' }}
                      onMouseEnter={e => { if (!sel) e.currentTarget.style.background = 'var(--s-850)' }}
                      onMouseLeave={e => { if (!sel) e.currentTarget.style.background = 'transparent' }}>
                      <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 transition-opacity ${sel ? '' : 'opacity-30'}`} style={{ background: m.color }} />
      ...
      <div className="grid grid-cols-4 gap-2 mb-5">
        {selectedDefs.map(m => {
          ...
          return (
            <button key={m.key as string} onClick={() => setActiveKey(m.key as string)} className="rounded-xl px-3 py-2.5 text-left transition-all"
              style={on ? { border: `1px solid ${m.color}60`, background: m.color + '12' } : { border: '1px solid var(--t-border)', background: 'var(--s-800)' }}>
              <div className="flex items-center gap-1.5 mb-1">
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: m.color }} />
                <span className="text-[11px] font-medium truncate" style={{ color: on ? m.color : 'var(--t-3)' }}>{m.label}</span>
              </div>
              <p className="text-sm font-bold truncate" style={{ color: on ? m.color : 'var(--t-1)' }}>{fmtPill(m)}</p>
            </button>
          )
        })}
      </div>
      <ResponsiveContainer width="100%" height={260}>
        <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          ...
          <XAxis dataKey="label" tick={{ fill: 'var(--t-3)', fontSize: 11 }} axisLine={false} tickLine={false} dy={8} />
          <YAxis tick={{ fill: 'var(--t-3)', fontSize: 11 }} axisLine={false} tickLine={false} width={62} tickFormatter={fmtTick} />
          <RechartsTooltip cursor={{ stroke: activeDef?.color, strokeWidth: 1, strokeOpacity: 0.3 }} content={({ active, payload, label }) => {
            if (!active || !payload?.length) return null
            const val = payload[0]?.value as number
            return (
              <div className="rounded-xl px-3.5 py-2.5 shadow-xl" style={{ background: 'var(--s-900)', border: '1px solid var(--t-border)' }}>
          ...
          <Area type="monotone" dataKey={activeKey} stroke={activeDef?.color} strokeWidth={2.5} fill="url(#activeGrad)" dot={false} activeDot={{ r: 5, strokeWidth: 2, stroke: activeDef?.color, fill: '#13151f' }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
...
export default function DashboardPage({ params }: { params: { workspaceId: string } }) {
  ...
  useEffect(() => {
    ...
    if (savedLabels) { try { setCustomLabels(JSON.parse(savedLabels) as Record<string, string>) } catch {} }
  }, [workspaceId])
  ...
  useEffect(() => {
    ...
    if (saved) { try { const parsed = JSON.parse(saved) as string[]; setMetricOrder([...parsed, ...DEFAULT_IDS.filter(id => !parsed.includes(id))]) } catch {} }
  }, [workspaceId])

  const saveOrder = (order: string[]) => { setMetricOrder(order); localStorage.setItem(`dash-metric-order-${workspaceId}`, JSON.stringify(order)) }

  useEffect(() => {
    fetch(`/api/meta/campaigns-list?workspaceId=${workspaceId}`).then(r => r.json()).then(j => { if (j.success) setCampaigns(j.data) }).catch(() => {})
  }, [workspaceId])
  ...
  const fetchData = useCallback(async (force = false) => {
    ...
    if (!force && cached && Date.now() - cached.ts < CACHE_TTL_MS) {
      setInsights(cached.insights); setDaily(cached.daily); setPrevOverview(cached.prevOverview); return
    }
    setLoading(true); setError(null)
    try {
      const prev   = getPreviousPeriod(startDate, endDate)
      const params = new URLSearchParams({ startDate, endDate, workspaceId, ...(selectedIds.length ? { campaignIds: selectedIds.join(',') } : {}) })
      const prevParams = new URLSearchParams({ startDate: prev.start, endDate: prev.end, workspaceId, ...(selectedIds.length ? { campaignIds: selectedIds.join(',') } : {}) })
      ...
      const [overviewJson, dailyJson, prevJson] = await Promise.all([overviewRes.json(), dailyRes.json(), prevRes.json()])
      ...
      setInsights(overviewJson.data); setDaily(dailyJson.data); setPrevOverview(prevOverviewData)
      dashCache.set(key, { insights: overviewJson.data, daily: dailyJson.data, prevOverview: prevOverviewData, ts: Date.now() })
    } catch (e) {
  ...
  const fetchCreatives = useCallback(async () => {
    ...
    if (cached && Date.now() - cached.ts < CACHE_TTL_MS) { setCreatives(cached.data); setCreativesLoading(false); return }
    setCreativesLoading(true); setCreativesError(null)
    try {
      const params = new URLSearchParams({ startDate, endDate, workspaceId, ...(selectedIds.length ? { campaignIds: selectedIds.join(',') } : {}) })
      ...
      setCreativesError(msg); toast.error(msg)
    } finally { setCreativesLoading(false) }
  }, [startDate, endDate, selectedIds, workspaceId])

  useEffect(() => { fetchCreatives() }, [fetchCreatives])
  useEffect(() => { if (tab === 'creatives') fetchCreatives() }, [tab, fetchCreatives])

  const openCreativePreview = useCallback(async (ad: AdInsight) => {
    setPreviewAd(ad); setPreviewDetail(null); setPreviewLoading(true)
    try {
    ...
    } catch { /* non-fatal */ } finally { setPreviewLoading(false) }
  }, [workspaceId])
  ...
  const handleDrop      = (targetId: string) => {
    ...
    const from = newOrder.indexOf(draggedId), to = newOrder.indexOf(targetId)
    newOrder.splice(from, 1); newOrder.splice(to, 0, draggedId)
    saveOrder(newOrder); setDraggedId(null); setDragOverId(null)
  }
  ...
  const visibleMetrics = metricOrder.map(id => ALL_METRICS.find(m => m.id === id)).filter((m): m is MetricDef => !!m)
    ...
    <div className="min-h-screen bg-surface-900 text-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            <Link href={`/dashboard/${workspaceId}/report`} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-700 border border-surface-600 text-sm font-medium text-gray-300 hover:text-gray-100 hover:bg-surface-600 transition-colors">
              <FileText className="w-4 h-4" />Relatório PDF
            </Link>
            <button onClick={() => fetchData(true)} disabled={loading} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-sm font-medium text-white transition-colors disabled:opacity-50">
        ...
        <div className="flex gap-1 p-1 bg-surface-800 rounded-xl w-fit">
          {[{ id: 'overview', label: 'Visão Geral', icon: LayoutDashboard }, { id: 'daily', label: 'Desempenho Diário', icon: TableProperties }, { id: 'creatives', label: 'Criativos', icon: ImageIcon }, { id: 'analytics', label: 'Web Analytics', icon: BarChart3 }].map(t => (
            <button key={t.id} onClick={() => setTab(t.id as typeof tab)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.id ? 'bg-surface-700 text-gray-100' : 'text-gray-500 hover:text-gray-300'}`}>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <DateRangePicker startDate={startDate} endDate={endDate} activePreset={activePreset} onChange={(s, e, p) => { setStartDate(s); setEndDate(e); setPreset(p ?? '') }} />
          <CampaignFilter campaigns={campaigns} selected={selectedIds} onChange={setSelectedIds} />
          {tab === 'creatives' && (
            <div className="ml-auto flex items-center gap-2">
              <button onClick={() => setGroupCreatives(g => !g)} className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm transition-colors ${groupCreatives ? 'bg-indigo-600/20 text-indigo-300 border-indigo-500/30' : 'bg-surface-800 text-gray-400 border-surface-700 hover:text-gray-200 hover:border-surface-600'}`}>
                <Layers className="w-4 h-4" />{groupCreatives ? 'Agrupado por nome' : 'Individual por conjunto'}
              </button>
              <button id="creatives-csv-btn" className="flex items-center gap-2 px-3 py-2 rounded-xl border text-sm bg-surface-800 text-gray-400 border-surface-700 hover:text-gray-200 hover:border-surface-600 transition-colors">
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {visibleMetrics.map(metric => (
                    <MetricCard key={metric.id} metric={metric} metricId={metric.id}
                      value={aggData ? metric.getValue(aggData) : 0}
                      prevValue={prevOverview ? metric.getValue(prevOverview) : undefined}
                      customLabel={customLabels[metric.id]} onLabelChange={updateCustomLabel}
                      isDragging={draggedId === metric.id} isOver={dragOverId === metric.id && draggedId !== metric.id}
                      onDragStart={() => handleDragStart(metric.id)} onDragOver={e => handleDragOver(e, metric.id)}
                      onDrop={() => handleDrop(metric.id)} onDragEnd={handleDragEnd} onRemove={() => removeMetric(metric.id)}
                    />
                  ))}
                  <button onClick={() => setShowAddModal(true)} className="border-2 border-dashed border-surface-700 rounded-2xl p-5 flex flex-col items-center justify-center gap-2 text-gray-600 hover:border-indigo-500/50 hover:text-indigo-400 transition-colors min-h-[120px]">
                    <Plus className="w-6 h-6" /><span className="text-xs font-medium">Adicionar métrica</span>
                  </button>
                </div>
                ...
                {!loading && daily.length > 0 && <PaceSection daily={daily} endDate={endDate} />}
                {insights.length > 0 && (() => {
                  const byId = new Map<string, typeof insights>()
                  for (const r of insights) { if (!byId.has(r.campaignId)) byId.set(r.campaignId, []); byId.get(r.campaignId)!.push(r) }
                  const rows = Array.from(byId.values()).map(rs => {
                    const spend=rs.reduce((s,r)=>s+r.spend,0), impressions=rs.reduce((s,r)=>s+r.impressions,0), reach=rs.reduce((s,r)=>s+r.reach,0), clicks=rs.reduce((s,r)=>s+r.clicks,0)
                    const purchases=rs.reduce((s,r)=>s+r.purchases,0), leads=rs.reduce((s,r)=>s+r.leads,0), initiateCheckout=rs.reduce((s,r)=>s+r.initiateCheckout,0)
                    const revenue=rs.reduce((s,r)=>s+r.revenue,0), landingPageViews=rs.reduce((s,r)=>s+r.landingPageViews,0)
                    return { campaignId: rs[0].campaignId, campaignName: rs[0].campaignName, spend, impressions, reach, clicks, purchases, leads, initiateCheckout, revenue, landingPageViews,
                      roas: spend>0?revenue/spend:0, ctr: impressions>0?(clicks/impressions)*100:0, cpm: impressions>0?(spend/impressions)*1000:0,
                      frequency: reach>0?impressions/reach:0, connectRate: clicks>0?(landingPageViews/clicks)*100:0,
                      purchaseRate: landingPageViews>0?(purchases/landingPageViews)*100:0, leadRate: landingPageViews>0?(leads/landingPageViews)*100:0,
                      cpc: clicks>0?spend/clicks:0, costPerLead: leads>0?spend/leads:0, costPerPurchase: purchases>0?spend/purchases:0,
                      costPerInitiateCheckout: initiateCheckout>0?spend/initiateCheckout:0 }
                    ...
                    <div className="bg-surface-800 border border-surface-700 rounded-2xl overflow-hidden mt-2">
                      <div className="px-5 py-3 border-b border-surface-700 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button onClick={() => exportCampaignsXLSX(rows, startDate, endDate)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface-700 border border-surface-600 text-xs text-gray-300 hover:text-gray-100 hover:border-surface-500 transition-colors">
                        ...
                        <table className="w-full text-sm">
                          <thead className="sticky top-0 z-10 border-b border-surface-700 bg-surface-750">
                            <tr>
                              ...
                              {['Investido','ROAS','Receita','Compras','Custo/Compra','Tx Conv.','Leads','Custo/Lead','Tx Conv. Lead','CPC','CPM','CTR','Frequência','Alcance','Impressões','Connect Rate'].map(h => (
                          ...
                          <tbody className="divide-y divide-surface-700/50">
                            {rows.map((row, i) => (
                              <tr key={row.campaignId} className="hover:bg-surface-750/50 transition-colors">
                                <td className="px-4 py-3"><div className="flex items-center gap-2.5"><span className="text-[11px] font-medium text-gray-600 w-5 text-center flex-shrink-0">{i+1}</span><span className="text-gray-200 text-sm font-medium truncate max-w-[280px]" title={row.campaignName}>{row.campaignName}</span></div></td>
                ...
                {!loading && <TimeBreakdown workspaceId={workspaceId} startDate={startDate} endDate={endDate} campaignIds={selectedIds} />}
              </>
            )}
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {!loading && sortedDaily.length > 0 && (
              <div className="flex justify-end">
                <button onClick={() => exportCSV(sortedDaily)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-800 border border-surface-700 text-sm text-gray-300 hover:border-surface-600 hover:text-gray-200 transition-colors">
            ...
            <div className="bg-surface-800 border border-surface-700 rounded-2xl overflow-hidden">
              {loading ? (
                ...
                <div className="text-center py-16 text-gray-500"><TableProperties className="w-10 h-10 mx-auto mb-3 opacity-30" /><p className="text-sm">Nenhum dado diário encontrado.</p></div>
              ) : (
                    ...
                    <tbody className="divide-y divide-surface-700/50">
                      {sortedDaily.map((row, i) => (
                        <tr key={i} className="hover:bg-surface-750/50 transition-colors">
                          <td className="px-4 py-3 text-gray-400 whitespace-nowrap font-mono text-xs">{row.date ? new Date(row.date+'T00:00:00').toLocaleDateString('pt-BR') : '—'}</td>
        ...
        {tab === 'creatives' && (() => {
          const displayRows = (() => {
            ...
            for (const row of creatives) { if (!map.has(row.adName)) map.set(row.adName, []); map.get(row.adName)!.push(row) }
            return Array.from(map.values()).map(rows => {
              const spend=rows.reduce((s,r)=>s+r.spend,0), impressions=rows.reduce((s,r)=>s+r.impressions,0), reach=rows.reduce((s,r)=>s+r.reach,0), clicks=rows.reduce((s,r)=>s+r.clicks,0)
              const purchases=rows.reduce((s,r)=>s+r.purchases,0), leads=rows.reduce((s,r)=>s+r.leads,0), initiateCheckout=rows.reduce((s,r)=>s+r.initiateCheckout,0)
              const revenue=rows.reduce((s,r)=>s+r.revenue,0), landingPageViews=rows.reduce((s,r)=>s+r.landingPageViews,0)
              return { ...rows[0], thumbnailUrl: rows.find(r=>r.thumbnailUrl)?.thumbnailUrl, adSetName: '', adSetId: '',
                spend, impressions, reach, clicks, purchases, leads, initiateCheckout, revenue, landingPageViews,
                roas: spend>0?revenue/spend:0, ctr: impressions>0?(clicks/impressions)*100:0, cpm: impressions>0?(spend/impressions)*1000:0,
                frequency: reach>0?impressions/reach:0, connectRate: clicks>0?(landingPageViews/clicks)*100:0,
                purchaseRate: landingPageViews>0?(purchases/landingPageViews)*100:0, leadRate: landingPageViews>0?(leads/landingPageViews)*100:0,
                cpc: clicks>0?spend/clicks:0, costPerLead: leads>0?spend/leads:0, costPerPurchase: purchases>0?spend/purchases:0,
          ...
          const sortedRows = [...displayRows].sort((a,b) => {
            ...
            const va=(a as unknown as Record<string,unknown>)[creativeSortKey], vb=(b as unknown as Record<string,unknown>)[creativeSortKey]
            return (Number(va??0)-Number(vb??0))*dir
          })
          const onSort = (k: string) => { if (k===creativeSortKey) setCreativeSortDir(d=>d==='asc'?'desc':'asc'); else { setCreativeSortKey(k); setCreativeSortDir('desc') } }
          const csvBtn = document.getElementById('creatives-csv-btn')
          if (csvBtn) {
            (csvBtn as HTMLButtonElement).onclick = () => {
              const headers = ['Nome','Campanha','Conjunto','Investido','ROAS','Receita','Compras','Custo/Compra','Tx Conv. Compra','Leads','Custo/Lead','Tx Conv. Lead','CPC','CPM','CTR','Frequência','Alcance','Impressões','LP Views','Connect Rate']
              const esc = (v: string|number) => { const s=String(v).replace(/"/g,'""'); return /[,"\n]/.test(s)?`"${s}"`:s }
              const lines = [headers.join(','), ...sortedRows.map(r => [esc(r.adName),esc(r.campaignName),esc(r.adSetName??''),r.spend.toFixed(2),r.roas.toFixed(2),r.revenue.toFixed(2),r.purchases,r.costPerPurchase>0?r.costPerPurchase.toFixed(2):'0',r.purchaseRate.toFixed(2),r.leads,r.costPerLead>0?r.costPerLead.toFixed(2):'0',r.leadRate.toFixed(2),r.cpc.toFixed(2),r.cpm.toFixed(2),r.ctr.toFixed(2),r.frequency.toFixed(2),r.reach,r.impressions,r.landingPageViews,r.connectRate.toFixed(2)].join(','))].join('\n')
              const blob=new Blob(['﻿'+lines],{type:'text/csv;charset=utf-8;'})
              const url=URL.createObjectURL(blob), a=document.createElement('a'); a.href=url; a.download=`criativos_${startDate}_${endDate}.csv`; a.click(); URL.revokeObjectURL(url)
            }
          }
          return (
            <div className="bg-surface-800 border border-surface-700 rounded-2xl overflow-hidden">
              {creativesLoading ? (
                ...
                <div className="text-center py-16 text-gray-500"><ImageIcon className="w-10 h-10 mx-auto mb-3 opacity-30" /><p className="text-sm">Nenhum criativo encontrado para o período selecionado.</p></div>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10 border-b border-surface-700 bg-surface-750">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500">{groupCreatives ? 'Criativo' : 'Criativo / Conjunto'}</th>
                        {([['spend','Investido'],['roas','ROAS'],['revenue','Receita'],['purchases','Compras'],['costPerPurchase','Custo/Compra'],['purchaseRate','Tx Conv.'],['leads','Leads'],['costPerLead','Custo/Lead'],['leadRate','Tx Conv. Lead'],['cpc','CPC'],['cpm','CPM'],['ctr','CTR'],['frequency','Freq.'],['impressions','Impressões'],['reach','Alcance']] as [string,string][]).map(([key,label]) => (
                          <Th key={key} label={label} sortKey={key} currentKey={creativeSortKey} dir={creativeSortDir} onSort={onSort} />
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-surface-700/50">
                      {sortedRows.map((row, i) => (
                        <tr key={i} className="hover:bg-surface-750/50 transition-colors">
                          <td className="px-4 py-2">
                            <button onClick={() => openCreativePreview(row)} className="block w-12 h-12 rounded-lg overflow-hidden bg-surface-700 flex-shrink-0 ring-0 hover:ring-2 hover:ring-indigo-500/60 transition-all group relative">
                              {row.thumbnailUrl ? (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img src={row.thumbnailUrl} alt="" className="w-full h-full object-cover" onError={e=>{(e.target as HTMLImageElement).style.display='none'}} />
                              ) : (
                                <div className="w-full h-full flex items-center justify-center"><ImageIcon className="w-5 h-5 text-gray-600" /></div>
                              )}
                              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Eye className="w-4 h-4 text-white" /></div>
                            </button>
                          </td>
                          <td className="px-4 py-2 max-w-[240px]">
                            <p className="text-gray-200 text-sm truncate font-medium" title={row.adName}>{row.adName}</p>
                            {groupCreatives ? null : <p className="text-gray-500 text-xs truncate mt-0.5" title={`${row.campaignName} / ${row.adSetName}`}>{row.campaignName}{row.adSetName?<span className="text-gray-600"> / {row.adSetName}</span>:null}</p>}
                          </td>
                          <td className="px-4 py-3 text-emerald-400 font-medium whitespace-nowrap">{fmtCurrency(row.spend)}</td>
                          <td className="px-4 py-3 text-gray-300 whitespace-nowrap">{fmtDecimal(row.roas)}</td>
                          <td className="px-4 py-3 text-gray-300 whitespace-nowrap">{fmtCurrency(row.revenue)}</td>
        ...
        {tab === 'analytics' && <GA4Section workspaceId={workspaceId} startDate={startDate} endDate={endDate} />}

      </div>

      {showAddModal && <AddMetricModal visibleIds={metricOrder} onAdd={addMetric} onClose={() => setShowAddModal(false)} />}

      {previewAd && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => { setPreviewAd(null); setPreviewDetail(null) }}>
          <div className="bg-surface-900 border border-surface-700 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-surface-700 flex-shrink-0">
              <p className="text-sm font-semibold text-gray-200 truncate pr-4" title={previewAd.adName}>{previewAd.adName}</p>
              <button onClick={() => { setPreviewAd(null); setPreviewDetail(null) }} className="text-gray-500 hover:text-gray-200 transition-colors flex-shrink-0"><X className="w-4 h-4" /></button>
            </div>
            <div className="flex flex-col sm:flex-row overflow-hidden flex-1 min-h-0">
              <div className="sm:w-64 flex-shrink-0 bg-black flex items-center justify-center min-h-[200px] relative">
                {previewLoading ? (
                  ...
                  <video key={previewDetail.videoUrl} src={`/api/meta/video-proxy?url=${encodeURIComponent(previewDetail.videoUrl)}`} poster={previewDetail.thumbUrl??previewAd.thumbnailUrl??undefined} controls preload="metadata" playsInline className="w-full h-full object-contain max-h-[70vh]" />
                ) : previewDetail?.isVideo ? (
                  <div className="flex flex-col items-center gap-3 p-4 text-center">
                    {(previewDetail.thumbUrl??previewAd.thumbnailUrl) && <img src={previewDetail.thumbUrl??previewAd.thumbnailUrl} alt="" className="w-full object-contain max-h-40 opacity-60" />}
                  ...
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={previewDetail?.imageUrl??previewDetail?.thumbUrl??previewAd.thumbnailUrl} alt="" className="w-full h-full object-contain max-h-[70vh]" />
                ) : (
                  <div className="flex flex-col items-center gap-2 text-gray-700"><ImageIcon className="w-10 h-10" /><span className="text-xs">Sem mídia</span></div>
                )}
              </div>
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {previewLoading ? (
                  <div className="flex items-center gap-2 text-gray-500 text-sm"><Loader2 className="w-4 h-4 animate-spin" />Carregando detalhes...</div>
                ) : (
                  <>
                    {previewDetail?.title && <div><p className="text-[10px] font-semibold text-gray-600 uppercase tracking-wider mb-1">Título</p><p className="text-sm text-gray-200 leading-relaxed">{previewDetail.title}</p></div>}
                    {previewDetail?.body && <div><p className="text-[10px] font-semibold text-gray-600 uppercase tracking-wider mb-1">Texto do anúncio</p><p className="text-sm text-gray-300 leading-relaxed whitespace-pre-wrap">{previewDetail.body}</p></div>}
                    {!previewDetail?.title && !previewDetail?.body && !previewLoading && <p className="text-xs text-gray-600 italic">Textos não disponíveis para este criativo.</p>}
                    <div className="pt-2 border-t border-surface-700 grid grid-cols-2 gap-x-4 gap-y-2">
                      <div><p className="text-[10px] text-gray-600">Investido</p><p className="text-sm font-semibold text-emerald-400">{fmtCurrency(previewAd.spend)}</p></div>
                      <div><p className="text-[10px] text-gray-600">ROAS</p><p className="text-sm font-semibold text-gray-200">{fmtDecimal(previewAd.roas)}</p></div>
                      <div><p className="text-[10px] text-gray-600">Compras</p><p className="text-sm font-semibold text-gray-200">{fmtNumber(previewAd.purchases)}</p></div>
                      <div><p className="text-[10px] text-gray-600">CTR</p><p className="text-sm font-semibold text-gray-200">{fmtPercent(previewAd.ctr)}</p></div>
                    </div>
                  </>
                )}
              </div>
