  ...
  return (
    <div className="min-h-screen bg-surface-900 text-gray-100 flex h-screen overflow-hidden">
            ...
            {data.map(row => (
              <button key={row.ym} onClick={() => setActiveMonth(row.ym)}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center justify-between gap-2
                  ${activeMonth === row.ym ? 'bg-indigo-600 text-white font-semibold' : row.error ? 'text-red-400 hover:text-red-300 hover:bg-surface-700' : 'text-gray-400 hover:text-gray-200 hover:bg-surface-700'}`}>
                <span className="truncate">{row.month}</span>
                {row.error ? <span className="text-[9px] opacity-70 shrink-0">erro</span> : row.spend > 0 && <span className="text-[9px] tabular-nums opacity-60 shrink-0">{R$(row.spend)}</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto min-w-0">
        {loading && (<div className="flex items-center justify-center h-full"><RefreshCw className="w-6 h-6 animate-spin text-indigo-400" /><span className="ml-3 text-gray-400">Buscando dados mês a mês…</span></div>)}
        {error && (<div className="m-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-sm text-red-400">{error}</div>)}
        {!loading && displayRow && (
          <div className="px-6 py-5">
            <div className="mb-5">
              <h2 className="text-xl font-bold text-gray-100">{activeMonth ? displayRow.month : 'Resumo do Período'}</h2>
              <p className="text-xs text-gray-500 mt-0.5">{activeMonth ? 'Desempenho mensal detalhado' : `${data.length} meses · ${startMonth} → ${endMonth}`}</p>
            </div>
            {displayRow.error && (<div className="mb-4 p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-sm text-red-400">Erro ao buscar dados deste mês: {displayRow.errorMsg ?? 'Falha na API da Meta'}</div>)}
            <div className="grid grid-cols-5 gap-3 mb-4">{HERO_METRICS.map(m => (<Card key={m.key} label={m.label} value={val(displayRow, m)} accent />))}</div>
            <div className="grid grid-cols-5 gap-2.5 mb-6">{REST_METRICS.map(m => (<Card key={m.key} label={m.label} value={val(displayRow, m)} />))}</div>
            {!activeMonth && total && (
              <div className="bg-surface-800 border border-surface-700 rounded-2xl p-5 mb-5">
                ...
                <div className="space-y-2">{data.map(row => (<BarRow key={row.ym} label={row.month} value={row.spend} max={maxSpend} fmt={R$} />))}</div>
              </div>
            )}
            {!activeMonth && total && (
              <div className="bg-surface-800 border border-surface-700 rounded-2xl overflow-hidden">
                <div className="px-5 py-3.5 border-b border-surface-700"><h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Comparativo Mensal</h3></div>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead><tr className="border-b border-surface-700"><th className="text-left px-4 py-2.5 text-gray-500 font-medium">Mês</th>{HERO_METRICS.map(m => (<th key={m.key} className="text-right px-3 py-2.5 text-gray-500 font-medium whitespace-nowrap">{m.label}</th>))}</tr></thead>
                    <tbody>
                      {data.map((row, i) => (<tr key={row.ym} onClick={() => setActiveMonth(row.ym)} className={`border-b border-surface-700/40 cursor-pointer hover:bg-surface-700/40 transition-colors ${i % 2 === 0 ? '' : 'bg-surface-700/20'} ${row.error ? 'opacity-50' : ''}`}><td className="px-4 py-2.5 font-medium text-gray-300">{row.month}</td>{HERO_METRICS.map(m => (<td key={m.key} className="text-right px-3 py-2.5 tabular-nums text-gray-400">{val(row, m)}</td>))}</tr>))}
                      <tr className="bg-indigo-500/10 border-t border-indigo-500/20"><td className="px-4 py-2.5 font-bold text-indigo-300">TOTAL</td>{HERO_METRICS.map(m => (<td key={m.key} className="text-right px-3 py-2.5 tabular-nums font-semibold text-indigo-200">{val(total, m)}</td>))}</tr>
                    </tbody>
                  </table>
                </div>
              </div>
