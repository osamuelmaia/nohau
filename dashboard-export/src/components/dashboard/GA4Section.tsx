  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <KpiCard label="Sessões" value={fmtNum(overview.sessions)} sub={`${fmtNum(overview.engagedSessions)} engajadas`} icon={Activity} color="text-indigo-400" />
        <KpiCard label="Taxa de Engajamento" value={fmtPct(overview.engagementRate)} sub="sessões com interação real" icon={Users} color="text-cyan-400" />
        <KpiCard label="Conversões" value={fmtNum(overview.conversions)} sub={`${fmtPct(overview.sessionConversionRate)} das sessões`} icon={ShoppingCart} color="text-emerald-400" />
        <KpiCard label="Receita (GA4)" value={fmtCur(overview.revenue)} icon={TrendingUp} color="text-green-400" />
        <KpiCard label="Duração Média" value={fmtDur(overview.avgSessionDuration)} sub="por sessão" icon={Clock} color="text-yellow-400" />
      </div>
