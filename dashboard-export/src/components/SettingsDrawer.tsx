...
import { X, Eye, EyeOff, RefreshCw, CheckCircle2, XCircle, Trash2, ChevronDown, ChevronUp, Building2, Zap, BarChart3 } from 'lucide-react'
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              style={inputStyle}
            />
            <button onClick={() => setShowToken(v => !v)} className="absolute right-2.5 top-1/2 -translate-y-1/2" style={{ color: 'var(--t-3)' }}>
      ...
      <div>
        ...
        <input value={pageId} onChange={e => setPageId(e.target.value)} placeholder="Ex: 123456789" className={inputCls} style={inputStyle} />
      </div>

      <div>
        <FieldLabel>Conta de Anúncios</FieldLabel>
        {selAccountName && <p className="text-xs mb-1.5" style={{ color: 'var(--t-2)' }}>Atual: <strong>{selAccountName}</strong></p>}
        <div className="flex gap-2">
          {accounts.length > 0 ? (
            <select value={selAccount} onChange={e => {
          ...
          <button onClick={loadAccounts} disabled={loadingAcc || (!token.trim() && !hasToken)}
...
function AnalyticsSection({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="text-xs font-medium block mb-1.5" style={{ color: 'var(--t-2)' }}>Property ID</label>
        <input value={propertyId} onChange={e => setPropertyId(e.target.value)} placeholder="123456789" className={inputCls} style={inputStyle} />
      </div>
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <label className="text-xs font-medium" style={{ color: 'var(--t-2)' }}>Service Account JSON</label>
          <button onClick={() => setShowJson(v => !v)} className="text-xs" style={{ color: 'var(--t-3)' }}>{showJson ? 'Ocultar' : 'Mostrar'}</button>
        </div>
        <textarea value={serviceAccount} onChange={e => setServiceAccount(e.target.value)}
          onFocus={() => { if (serviceAccount.startsWith('•')) setServiceAccount('') }}
          rows={showJson ? 8 : 2} placeholder={'{"type":"service_account","project_id":"..."}'}
...
function WorkspaceSection({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-xs font-medium block mb-1.5" style={{ color: 'var(--t-2)' }}>Nome do workspace</label>
        <div className="flex gap-2">
...
export default function SettingsDrawer() {
  ...
  return (
    <>
      {open && <div className="fixed inset-0 z-40" style={{ background: 'rgba(0,0,0,0.3)' }} onClick={closeDrawer} />}
      <aside
        className={`fixed top-0 right-0 z-50 h-full flex flex-col transform transition-transform duration-250 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}
        style={{ width: 'min(420px, 100vw)', background: 'var(--s-900)', borderLeft: '1px solid var(--t-border)', boxShadow: '0 0 40px rgba(0,0,0,0.15)' }}>

        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0" style={{ borderBottom: '1px solid var(--t-border)' }}>
          <p className="text-sm font-semibold" style={{ color: 'var(--t-1)' }}>Configurações</p>
          <button onClick={closeDrawer} className="p-1.5 rounded-lg transition-colors" style={{ color: 'var(--t-3)' }}
