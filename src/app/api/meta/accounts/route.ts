...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Single-account validation mode
    if (adAccountId) {
      const normalized = adAccountId.startsWith('act_') ? adAccountId : `act_${adAccountId}`
      const account    = await getAdAccount(client, normalized)
    ...
    await prisma.apiLog.create({
      data: {
        operation: 'LIST_AD_ACCOUNTS', method: 'GET', endpoint: '/me/adaccounts',
        responseBody: JSON.stringify({ count: accounts.length }), statusCode: 200,
